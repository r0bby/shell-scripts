...
APPLICATION=$1
if [ "$APPLICATION" == "" ]; then
	echo "Manca il parametro Applicazione"
	exit
fi

exec >> /logs/archive/archive.$APPLICATION.log 2>&1
...
OBJRUNNING=*.log
OBJ=*.log.*
OBJPATH=/logs/IBMWebAS/log4j.$APPLICATION
KEEPPATH=/logs/IBMWebAS/log4j.$APPLICATION/latest
...
portserver=22022
path_log_server="/netapp/archiviazione_log/${CLIENT}${OBJPATH}"
...
if [ $APPLICATION == "ferwebi" ]
then echo "Ruoto i file di webi"
     ls -la *.log | while read i
